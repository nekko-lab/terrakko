...
- CI は `.github/workflows/publish.yaml` で GitHub Release の `published` イベント時に GHCR へ Docker イメージを publish する。リリース作成は `.github/workflows/release.yaml` が `main` push 時に PR ラベル（`minor` / `major` / デフォルト `patch`）を元にタグを自動計算して行う。
