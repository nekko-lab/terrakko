...
1. `app/src/terrakko.py`
   ...
   - `commands.Bot`（prefix: `trk` またはメンション）で UI フローを実装。
   - `View` / `Modal`（`StartButton`, `MainMenu`, `SelectVMNameTab`, `ConfirmAndExecute` など）で Create/Delete/Power 操作を分岐。
2. `app/src/proxmox_ve.py`
   - Proxmox API ラッパー。
   - モジュール import 時に `InitializePVEInfo()` を実行し、`pve`, `region`, `node`, `temp_id` を初期化。
   - VM 作成・起動・停止・削除・IP 取得を提供。
3. `app/src/config.py`
   - `.env` をロードして設定を集約。
   - `PVE_REGION` / `PVE_TEMP_ID` は JSON 配列として読み込み、同じインデックスで対応付ける。
...
1. **VM 所有権の識別は VM 名プレフィックス**
   - `GetNodeVM(author_id)` は `vm['name'].startswith(str(author_id))` で所有 VM を抽出する。
   - VM 名の命名規則（`<discord_user_id>-<name>`）を壊すと一覧・操作対象選別が機能しない。

2. **Proxmox ノードとテンプレート ID は起動時に 1 回選択**
   - `InitializePVEInfo()` で `PVE_REGION` からランダム選択し、同じ index の `PVE_TEMP_ID` を使う。
   - Bot プロセス稼働中は選択結果が固定される前提で実装されている。
...
4. **Discord 応答は Ephemeral 中心**
   ...
   - 長時間処理は `WaitForTaskCompletion()` と followup 通知の流れを踏襲する。
...
- 全体概要の補助: `README.md`, `docs/codebase-overview.md`
