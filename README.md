...
- `/vm build` — テンプレートから VM を作成（Cloud-init 対応）
- `/vm start` — 停止中の VM を起動
- `/vm stop` — 起動中の VM をシャットダウン
- `/vm status` — VM のステータス・IP アドレスを確認
- `/vm delete` — VM を削除（二重確認あり）
...
- DB なし。VM の所有権は PVE タグ（`discord_[uuid]`）で管理する。

### タグベース所有権

- VM 作成時にクローン完了直後に `discord_[uuid]` タグを自動付与。
- 各コマンドは自分の UUID に対応するタグを持つ VM のみ操作対象とする。
